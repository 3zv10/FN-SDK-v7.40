#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BP_ProjectileTrajectory_AthenaRed_Playset

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "BP_ProjectileTrajectory_Athena_SnapBoxToGrid_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass BP_ProjectileTrajectory_AthenaRed_Playset.BP_ProjectileTrajectory_AthenaRed_Playset_C
// 0x0010 (0x0420 - 0x0410)
class ABP_ProjectileTrajectory_AthenaRed_Playset_C final : public ABP_ProjectileTrajectory_Athena_SnapBoxToGrid_C
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame_BP_ProjectileTrajectory_AthenaRed_Playset_C; // 0x0410(0x0008)(ZeroConstructor, Transient, DuplicateTransient)
	class UAudioComponent*                        Creative_Grenade_LoopRed_Cue;                      // 0x0418(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)

public:
	void ExecuteUbergraph_BP_ProjectileTrajectory_AthenaRed_Playset(int32 EntryPoint);
	void ReceiveBeginPlay();
	void ReceiveDestroyed();
	void UserConstructionScript();

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"BP_ProjectileTrajectory_AthenaRed_Playset_C">();
	}
	static class ABP_ProjectileTrajectory_AthenaRed_Playset_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<ABP_ProjectileTrajectory_AthenaRed_Playset_C>();
	}
};
static_assert(alignof(ABP_ProjectileTrajectory_AthenaRed_Playset_C) == 0x000008, "Wrong alignment on ABP_ProjectileTrajectory_AthenaRed_Playset_C");
static_assert(sizeof(ABP_ProjectileTrajectory_AthenaRed_Playset_C) == 0x000420, "Wrong size on ABP_ProjectileTrajectory_AthenaRed_Playset_C");
static_assert(offsetof(ABP_ProjectileTrajectory_AthenaRed_Playset_C, UberGraphFrame_BP_ProjectileTrajectory_AthenaRed_Playset_C) == 0x000410, "Member 'ABP_ProjectileTrajectory_AthenaRed_Playset_C::UberGraphFrame_BP_ProjectileTrajectory_AthenaRed_Playset_C' has a wrong offset!");
static_assert(offsetof(ABP_ProjectileTrajectory_AthenaRed_Playset_C, Creative_Grenade_LoopRed_Cue) == 0x000418, "Member 'ABP_ProjectileTrajectory_AthenaRed_Playset_C::Creative_Grenade_LoopRed_Cue' has a wrong offset!");

}

